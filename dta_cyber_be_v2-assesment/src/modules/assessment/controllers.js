...
const {Quiz, QuizQuestion, QuizGroup, UserQuizTracker, QuizOption, QuizAnswer,ConcreteCases} = require("./models");
...
const assessmentListController = async (req, res, next) => {
    try {
        ...
        if (!isNull(req.user)) {
            ...
            console.log(trackers,"trackers",req.user,req.user.id)
        }
...
const ConcreteCasesController=async(req,res,next)=>{
    try {
        const content=await ConcreteCases.findAll({
            where:{
                
            }
        })
        console.log(content)
        apiOkResponse(res,content)
    } catch (error) {
        next(error)
    }
}
...
const assessmentStartController = async (req, res, next) => {
    try {
        ...
        const quiz = await getInstanceOrException(Quiz, req.params.quiz_id)
         console.log(quiz,req.params.quiz_id)
        // quiz groups
